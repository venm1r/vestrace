...
## 2026-08-16 Task 7 amendment: Compose bootstrap RED/GREEN

Human approval expands Task 7 to one Compose-only correction. Treat the captured fresh-volume `access_tokens_workspace_id_fkey` startup failure as RED. Add a one-shot migration service using the existing root image and runtime database identity; require `dev-seed` to wait for successful migration; require server and worker to wait for successful seed; keep worker independent of server health and keep console dependent on server health. Validate the resolved DAG, rerun proportionate deterministic gates, then run the full acceptance sequence under a new validated project and non-default ports with generated/cache before/after snapshots and finally cleanup. If ignored Compose tests expose a hardcoded default project or port, stop for context before changing their source. Commit only `docker-compose.yml`, this plan, the paired design amendment, and the durable evidence report.

Human resolution of the resulting context authorizes exactly two verification-harness corrections in the same isolated-acceptance scope. Use the observed HTTP 401 as RED for adding credential-safe bearer authorization to `foundation-run-smoke.sh`. Add pure parser tests RED-first for `compose_smoke.rs`, then make all health/metrics URLs honor a validated `VESTRACE_HTTP_PORT` while preserving its existing Compose project and cleanup commands. Rerun focused GREEN, fmt, workspace Clippy/no-run, and the entire fresh isolated sequence. The commit scope expands only to these two harness files in addition to the already approved Task 7 files.

The observed Git Bash exit 49 authorizes a bounded portability correction within `foundation-run-smoke.sh`: execute-check `python3` and then `python` for Python 3, fail early with a bounded diagnostic if neither works, and use the selected interpreter for both existing JSON assertion blocks without changing their logic. Rerun real isolated run smoke before continuing runtime RLS and ignored Compose tests.

The observed authenticated HTTP 200 resolves the final run-smoke context: replace the stale 404 expectation with two assertions. Other-workspace identity headers without a bearer must receive 401; the valid admin bearer with those same spoofed headers must receive 200 and return the created run ID, proving credential authority. Do not seed another token, weaken authentication, or treat this HTTP harness as a substitute for runtime RLS/database isolation evidence.

The subsequent ignored-suite 3/4 result authorizes only an authenticated metrics-harness correction in `tests/compose_smoke.rs`. Keep health probes unauthenticated; make both metrics tests use `VESTRACE_ADMIN_TOKEN` with the existing public local-development fallback; never include the token/header in diagnostics; and require HTTP success before metric-name or sensitive-label assertions. Rerun focused format/compile/parser checks and the complete fresh isolated acceptance chain before committing the six approved Task 7 paths.
