...
6. **Browser-blocked ledger (not a sub-package).** The browser conjuncts of g0-10, g0-11, g0-14 (and any found in P05-G/H) stay recorded as `blocked` with the exact missing oracle named, in whichever sub-package's manifest entry they belong to. g0-11 closed without a browser oracle (P05-G). Playwright reconnected 2026-09-18, but revisiting g0-10/g0-14 then found a second, deeper blocker that a browser tool alone does not resolve: `apps/console/src` renders nothing about EmbeddingSpaceTransition/TransitionBatch/RetrievalGenerationFence state (confirmed by source search, zero matches), so there is no UI surface for a browser scenario to observe — that surface is P06 scope (real Connections/Models/Runs), not a tooling gap. g0-10/g0-14 stay blocked, with their manifest reasons updated to name this precisely. Do not retry them again until P06 has built the relevant UI.
