...
Post-incident qualification now has a typed local evidence input and a fail-closed bundle lifecycle; deterministic recovery action evaluation, durable Incident/RevalidationRun/TrustState/RecoveryPoint and external effect/reconciliation persistence, startup recovery orchestration, workspace-scoped UNKNOWN-effect reconciliation, an injected deterministic effect fault-suite boundary, fault-evidence-backed qualification-bundle assembly, and an isolated subprocess fault-runtime boundary are available. Provider-specific execution, progressive trust restoration, and release approval remain open.
