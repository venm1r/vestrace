...
test('the bounded migrator is named by its exported path', () => {
  ...
  for (const file of users) {
    assert.match(
      ...
      // (bounded at 208). The P06 follow-up's Task 1 added
      // QUALIFICATION_WORK_CLAIMS_HISTORICAL_MIGRATOR, bounded at 217 for
      // the same reason. All three are legitimately exported by
      // vestrace_infrastructure. A file may use any one of them, or (like
      // intent_crash_boundaries.rs) more than one across different tests.
      /migrator = "vestrace_infrastructure::(HISTORICAL_MIGRATOR|DRAIN_HISTORICAL_MIGRATOR|QUALIFICATION_WORK_CLAIMS_HISTORICAL_MIGRATOR)"/,
